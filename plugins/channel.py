...
async def send_movie_updates(bot, file_name, file_id):
    ...
    if not poster_url or not caption:
        return
    btn = [
        [InlineKeyboardButton('Get File', url=f'@amarsmartindia_movieproviderbot?start=pm_mode_file_{ADMINS[0]}_{file_id}')]
    ]
    reply_markup = InlineKeyboardMarkup(btn)
    movie_update_channel =await db.movies_update_channel_i00t. (t.me/disscussiofilm/)
    try:
